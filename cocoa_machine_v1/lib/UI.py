...
# # Variable which use in multithread
...
#         self.main_widget = Main_widget(webcam, preprocessing, 
#                                   model_segment, 
#                                   model_classify, 
#                                   scheduler)
...
#         self.scheduler.tracker.update(classes, centroids)
#         ids = self.scheduler.tracker.IDs
...
#                 img_show[centroids[i][1]-4+y_offset:centroids[i][1]+4+y_offset, centroids[i][0]-4+x_offset:centroids[i][0]+4+x_offset] = [0, 255, 0]

#                 text = "ID:{} {} P:{:.2f}".format(ids[i] , self.model_classify.index2label[classes[i]], bean_prob[i][classes[i]])
